...
- 멤버: `프로필 탐색/필터 → 상세 → 마음 보내기 → 상대 수락 → 연락처 공개`
...
[멤버용](docs/guide/member.md) · [주선자용](docs/guide/admin.md) · [FAQ](docs/guide/faq.md)
...
| 역할   | 로그인                                                                 |
| ------ | ---------------------------------------------------------------------- |
| 주선자 | `admin@bolsaram.local` — **비밀번호 없음. 카카오·구글 로그인**          |
| 멤버   | `01020001000` ~ `01020001005` — **비밀번호 없음. 초대 링크로 들어갑니다** |

멤버 계정은 주선자 화면에서 프로필 상세 → 초대 링크를 발급해 그 링크로 로그인합니다.
자유 가입이 없고, 전화번호는 신원이 아니라 주선자가 기록하는 연락 수단입니다.

**주선자는 카카오·구글로만 로그인합니다** — 볼사람은 비밀번호를 받지도 저장하지도
않습니다. 개발 환경에서 로그인하려면 각 제공자 콘솔에서 키를 받아
...
- **인증** 자체 세션(서명 쿠키 + DB 세션). 주선자 이메일/비밀번호, **멤버는 초대 링크**
...
| 디렉터리             | 역할                                                        | 문서                                   |
| -------------------- | ----------------------------------------------------------- | -------------------------------------- |
| `apps/web`           | Next.js 앱 — 주선자 화면 + 멤버 화면 + API                  | [README](apps/web/README.md)           |
...
| `assets`             | 브랜드 에셋 원본 (로고·심볼·favicon) — `apps/web/public/` 로 복사해 쓴다 | [README](assets/README.md)   |
...
- **SMS 발송** — 쓰지 않기로 했습니다. 멤버 로그인은 주선자가 카카오톡으로 보내는
  초대 링크(매직 링크)입니다.
- **멤버 알림** — 보내지 않기로 했습니다. 주선자는 텔레그램으로 신청·수락 알림을 받고,
  멤버는 시그널 화면에서 직접 확인합니다. 연락이 필요한 일은 주선자가 카카오톡으로 합니다.
...
- **시드 데이터 정리** — 이 호스트에는 아직 합성 프로필이 떠 있습니다. 실제 멤버를 받기 전에
