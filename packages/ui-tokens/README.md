...
표면 클래스는 색 말고 레이아웃 치수도 하나 들고 있다. `.member-surface` 의
`--member-header-h` 는 회원 화면 상단 바 높이이고, 그 아래 붙는 sticky 하위 바
(성별 탭·시그널 탭)가 이 값을 읽어 자리를 잡는다. 헤더 안의 글자 크기를 바꾸면
이 변수도 같이 본다 — 숫자를 각자 들고 있으면 사이가 벌어지거나 겹친다.
