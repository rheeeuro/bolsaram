...
색·타이포·간격·모션 토큰. 볼사람은 주선자를 위한 서비스이고, 주선자 화면과 멤버 화면이
...
| 클래스            | 어디        | 성격                                       |
| ----------------- | ----------- | ------------------------------------------ |
| `.member-surface` | 멤버 화면   | ivory-50 바탕, rose 강조                   |
...
표면 클래스는 색 말고 레이아웃 치수도 들고 있다. `.member-surface` 의
`--member-header-h` 는 멤버 화면 상단 바 높이이고, 그 아래 붙는 sticky 하위 바
(시그널 탭·필터 줄)가 이 값을 읽어 자리를 잡는다. `.host-surface` 의
`--host-tabbar-h` 는 주선자 모바일 하단 탭 높이이고, 탭 자신과 그만큼 바닥을 비우는
레이아웃이 함께 읽는다(넓은 화면에는 탭이 없다). 둘 다 붙는 쪽이 숫자를 따로 들고
있으면 사이가 벌어지거나 겹친다 — 높이를 바꿀 때는 이 변수만 고친다.
