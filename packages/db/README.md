...
`cli/seed.ts` 는 `db/sample/` 을 읽어 **주선자 두 명 · 모임 하나 · 그 모임의 프로필**을
만든다. 디렉터리 하나가 사람 하나(`profile.txt` + 사진)이고, 이름 앞 번호로 갈라
앞쪽은 `SEED_ADMIN_EMAIL1`, 뒤쪽은 `SEED_ADMIN_EMAIL2` 명의로 등록한다.

| 환경변수                                | 하는 일                                   |
| --------------------------------------- | ----------------------------------------- |
| `SEED_ADMIN_EMAIL1` `SEED_ADMIN_EMAIL2` | 주선자 두 명의 이메일. **둘 다 필수**     |
| `SEED_SAMPLE_DIR`                       | 샘플 위치 (기본 `db/sample`, git 제외)    |
| `SEED_SPLIT_AT`                         | 1번이 맡는 마지막 번호 (기본 10)          |
| `SEED_FORCE=yes`                        | 데이터가 있어도 강행                      |

프로필마다 **등록된 가져오기 세션**(`IMPORTED`)을 하나씩 남긴다 — 원문(`profile.txt`),
사진 에셋, 원문에서 뽑은 추출 결과(모델 이름 `seed`)까지. 운영에서 프로필은 모두 이
경로로 들어오고 「원본 보기」와 검토 화면이 프로필이 아니라 이 세션을 읽는다.
에셋은 프로필 사진과 **같은 저장 키**를 가리킨다 — 실제 commit 경로도 파일을 복사하지
않고 키를 물려준다.

만드는 프로필에는 `is_seed = true` 가 붙는다 — `pnpm db:purge-seed` 가 이것만 걷어낸다.
그 세션도 함께 지운다. **프로필보다 먼저** 지워야 한다 — `committed_profile_id` 가
`SET NULL` 로 풀리면 `import_sessions_commit_pair` 에 걸려 삭제가 실패한다.
사진은 샘플에서 private 스토리지로 복사하고, 저장 키에 난수를 넣는다.
**프로필 테이블이 비어 있을 때만** 스토리지의 `profile/` · `import/` 를 함께 비운다 —
데이터가 남아 있는 상태에서는 파일을 지우지 않는다.

멤버 계정은 만들지 않는다. 멤버 화면은 주선자 화면에서 초대 링크를 발급해 확인한다.
