...
- 회원 흐름: `프로필 탐색/필터 → 상세 → 마음 보내기 → 상대 수락 → 연락처 공개`
...
| 인증     | 자체 세션(서명 쿠키 + `sessions`). **주선자는 카카오·구글 OAuth**, 회원은 초대 링크 |
...
- **시드 데이터 정리**: 이 호스트에는 아직 합성 프로필이 떠 있습니다.
  실회원을 받기 전에 `pnpm db:purge-seed --yes` 로 걷어내세요(표식은 `profiles.is_seed`).
