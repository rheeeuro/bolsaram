...
| 인증     | 자체 세션(서명 쿠키 + `sessions`). **주선자는 카카오·구글 OAuth**, 멤버는 초대 링크 |
...
pnpm dev:session       # 점검용 주선자 세션 쿠키 (개발 전용, production 거절)
...
멤버 계정은 **초대 링크를 소비할 때만** 만들어집니다(`consumeInvite`).
자유 가입이 없고, 전화번호는 신원이 아니라 주선자가 기록하는 연락 수단입니다.

시드(`pnpm db:seed`)가 만드는 것:

- 주선자 두 명 — `.env` 의 `SEED_ADMIN_EMAIL1` · `SEED_ADMIN_EMAIL2`. **둘 다 있어야
  돕니다.** **비밀번호가 없습니다** — 카카오·구글로만 로그인하고(0050) 첫 로그인에서
  같은 이메일의 계정에 이어붙습니다. 화면 동작만 확인할 때는 `pnpm dev:session` 이
  점검용 세션 쿠키를 발급합니다.
- 모임 「애니메이션 모임」 — 1번이 만들고(모임장) 2번이 합류합니다.
- 프로필 — `db/sample/` 의 디렉터리 하나가 사람 하나입니다(`profile.txt` + 사진).
  디렉터리 이름 앞 번호로 갈라 앞쪽은 1번, 뒤쪽은 2번 명의로 등록하고(경계는
  `SEED_SPLIT_AT`, 기본 10) 사진은 private 스토리지로 복사합니다. 전부 공개 상태입니다.
- 가져오기 원본 — 프로필마다 등록된 Import 세션 하나. `profile.txt` 원문과 사진,
  그 원문에서 뽑은 항목이 들어갑니다. 운영에서 프로필은 모두 이 경로로 들어오므로
  주선자 화면의 「원본 보기」와 가져오기 검토 화면이 시드에서도 그대로 동작합니다.
- **멤버 계정은 만들지 않습니다.** 멤버 화면을 보려면 주선자 화면에서 프로필 상세 →
  초대 링크를 발급해 그 링크로 들어갑니다(매직 링크).

`db/sample/` 은 git 에 올라가지 않습니다. 없으면 시드가 이유를 말하고 멈춥니다
(위치는 `SEED_SAMPLE_DIR` 로 바꿉니다). 프로필 글에 없는 성별은 시드가 번호로 지정하고,
직업군은 키워드로 추정합니다 — 운영에서는 AI 가 뽑고 주선자가 고치는 값입니다.
...
```
apps/web/src/
  app/            라우트. (member) 그룹 = 멤버, (host) 그룹 = 주선자, api/ = Route Handler
...
db/sample/        시드가 읽는 샘플 프로필 (git 제외)
tests/            vitest. 도메인 단위 + DB 통합
.agent-config/    에이전트 하네스 원본 (위 참고)
scripts/          운영 스크립트 (DB 백업, 시드 정리, webhook 등록, AI 키 확인, 가동 감시, 점검 세션)
...
`docs/guide/` 는 **멤버와 주선자가 읽는 문서**다. 개발자 문서가 아니다.

| 파일        | 대상                                   |
| ----------- | -------------------------------------- |
| `README.md` | 인덱스와 전체 흐름                     |
| `member.md` | 멤버 — 로그인, 탐색, 신청, 시그널      |
...
멤버·주선자 화면, 인증(`server/auth/`), 상태 enum, 업로드 제한, PM2 스케줄.
...
- 정책 숫자(인증번호 5분·재요청 30초·시도 5회·로그인 30일·초대 72시간·업로드 25MB·
  프로필 사진 5장·분석 6장·신뢰도 65%·정리 04:10·기본 필터 범위)가 코드와 같은가
...
- **모임 소속(`group_admins`)을 바꾸는 것은 인증 레이어만 합니다.** 정책을 주지 않았고
  owner 커넥션의 가입·초대 경로만이 소속을 만듭니다. 한 주선자는 **여러 모임에
  동시에 속합니다** — 정책은 속한 모임 전부를 통과시킵니다.
- **보고 있는 모임(`users.active_group_id`)은 화면 필터이지 권한이 아닙니다.**
  RLS 는 이 값을 보지 않습니다. 주선자 목록만 이 값으로 좁히고, 무엇을 볼 수 있는지는
  `group_admins` 가 정합니다. 런타임 롤에는 이 컬럼의 UPDATE 권한이 없습니다.
...
- **SMS**: 쓰지 않기로 했습니다(2026-09-07). 멤버 로그인은 주선자가 카카오톡으로 보내는
  **초대 링크**입니다 — 링크가 세션을 만들고, 처음이면 멤버 계정도 함께 만듭니다.
  전화번호 OTP 경로·`server/sms/`·`DEV_EXPOSE_OTP` 는 제거했습니다(0015).
  세션(30일)이 만료되면 주선자가 링크를 재발급합니다.
- **모임 사이 프로필 이동**: 없습니다. 주선자는 여러 모임에 동시에 속하고 상단
  전환기로 오갑니다(전체공개도 하나의 채널입니다). 하지만 **프로필을 다른 모임으로
  옮기는 기능은 없습니다** — 마지막 주선자가 나갈 때 막히는 이유가 이것입니다.
- **주선자 비밀번호**: 없앴습니다(2026-09-18). 카카오·구글 OAuth 만 받고 비밀번호
  컬럼과 시도 제한 테이블을 지웠습니다(0050). 남은 위험은 사람 쪽입니다 — 주선자
  계정의 안전이 그 소셜 계정의 보안 설정에 달려 있고, 우리는 2단계 인증을 강제할 수
  없습니다. **제공자 키가 하나도 없으면 주선자가 로그인할 수 없습니다**(기동 로그에
  경고가 남습니다).
- **개인정보 처리방침 확정**: `docs/guide/privacy.md` 는 코드 기준 초안이며 **법률 검토를
  받지 않았습니다.** 등록되는 사람의 동의는 **주선자가 받는다고 전제**하고 시스템이
  기록하지 않습니다(0019 를 0020 에서 되돌렸습니다). 운영 형태(개인 운영)·보호책임자·
  문의 창구·철회 절차는 채웠고 `/privacy` 가 `docs/guide/privacy.md` 를 그대로
  보여줍니다(로그인 불필요). 남은 것은 **법률 검토와 시행일** 입니다 — 화면에
  「법률 검토를 받지 않았습니다」 경고가 그대로 보입니다.
- **멤버 알림**: 보내지 않기로 했습니다(2026-09-08). 주선자만 텔레그램으로 받고
  (`server/notify/`), 멤버는 시그널 화면에서 직접 확인합니다. 멤버용 채널을 새로
...
- **시드 데이터 정리**: 이 호스트에는 아직 합성 프로필이 떠 있습니다.
  실제 멤버를 받기 전에 `pnpm db:purge-seed --yes` 로 걷어내세요(표식은 `profiles.is_seed`).
