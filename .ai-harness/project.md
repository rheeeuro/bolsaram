...
- **모임이 최상위 격리 단위입니다.** 주선자 가입은 열려 있고, 가입하면 자기 모임이
  생깁니다. 회원·Import·신청이 모두 모임 안에 갇히며 모임을 넘지 않습니다.
...
- **`ADMIN` 이라는 사실만으로 권한을 주지 않습니다.** 관리자 판정은 항상
  `app_is_group_admin(group_id)` 이며, 데이터를 만들거나 고치는 경로는
  `asGroupAdmin` / `requireAdminGroup` 을 씁니다. 새 행에는 `group_id` 를 반드시 넣습니다.
...
- **동료 주선자 초대**: 없습니다. 각자 가입하면 각자 모임이 생겨서 **같은 모임에서 함께
  일할 방법이 없습니다.** 모임 초대 코드(텔레그램 연결 코드와 같은 방식)가 필요합니다.
- **주선자 비밀번호**: 시드 값(`bolsaram-admin`)이 문서화되어 있습니다. 공개 도메인에
  열려 있으므로 **운영 전에 반드시 교체**합니다. 시도 제한은 15분 안 5회로 걸려 있지만
  이미 알려진 비밀번호는 막지 못합니다.
