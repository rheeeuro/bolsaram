...
- 주선자 `admin@bolsaram.local` — **비밀번호가 없습니다.** 주선자는 카카오·구글로만
  로그인합니다(0050). 이 계정으로 직접 들어가려면 `SEED_ADMIN_EMAIL` 에 본인 소셜
  계정 이메일을 넣고 시드합니다 — 첫 로그인에서 같은 이메일의 계정에 이어붙습니다.
  화면 동작만 확인할 때는 `pnpm dev:session` 이 점검용 세션 쿠키를 발급합니다.
- 멤버 `01020001000` ~ `01020001005` — **비밀번호가 없습니다.** 주선자 화면에서
