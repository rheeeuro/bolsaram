...
7. 파일을 추가·삭제·이동했으면 그 디렉터리의 `README.md` 를 함께 고친다.
   사용자에게 보이는 동작·정책이 바뀌었으면 `docs/guide/` 도 고친다.
   두 문서의 어긋남은 `tests/docs-readme.test.ts` · `tests/docs-guide.test.ts` 가
   잡아준다(둘 다 `pnpm verify` 에 포함).
8. 실패를 고치고 같은 검증을 다시 통과시킨 뒤에만 완료로 보고한다.
