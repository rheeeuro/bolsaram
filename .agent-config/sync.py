...
def deployment_hooks(directory: str) -> dict:
    """두 클라이언트의 시작·종료 이벤트는 같은 스크립트와 제한 시간을 쓴다."""
    return {
        event: [{"hooks": [{
            "type": "command",
            "command": f'/usr/bin/env bash "{directory}/{script}"',
            "timeout": timeout,
        }]}]
        for event, script, timeout in [
            ("UserPromptSubmit", "mark-turn-start.sh", 30),
            ("Stop", "deploy-on-stop.sh", 600),
        ]
    }
...
def claude_settings(manifest: dict) -> dict:
    ...
    if flags["deploy_on_stop"]:
        hooks.update(deployment_hooks("$CLAUDE_PROJECT_DIR/.claude/hooks"))
        hooks.setdefault("PostToolUse", []).append({
            "matcher": "Edit|Write",
            "hooks": [{
                "type": "command",
                "command": '/usr/bin/env bash "$CLAUDE_PROJECT_DIR/.claude/hooks/track-changes.sh"',
                "timeout": 30,
            }],
        })
...
def codex_hooks(manifest: dict) -> dict:
    ...
    if flags["deploy_on_stop"]:
        hooks.update(deployment_hooks(f"{root}/.claude/hooks"))
        hooks.setdefault("PostToolUse", []).append({
            "matcher": "Edit|Write",
            "hooks": [{
                "type": "command",
                "command": f'/usr/bin/env bash "{root}/.claude/hooks/track-changes.sh"',
                "timeout": 30,
                "statusMessage": "배포할 변경 파일 기록",
            }],
        })
