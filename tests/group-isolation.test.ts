 ...
 * 이전에는 관리자 정책이 `app_is_admin()` 하나였고, ADMIN 계정 하나로 전체 멤버의
 * 이름·연락처·사진에 접근할 수 있었다.
 *
 * 여기서 지키는 성질 — 모두 "안 되는 것"이다.
 *   * 다른 모임의 주선자는 남의 멤버를 읽지도 고치지도 못한다.
 *   * 멤버는 자기 모임 안의 프로필만 본다.
...
describe("주선자는 자기 모임만 다룬다", () => {
  ...
  /**
   * 초대 발급은 owner 커넥션을 쓰므로 위 정책을 지나간다. 초대 링크는 곧 그 멤버의
