...
# 멤버가 처음 만나는 화면이 문서와 어긋난 채로 남는다(실제로 그랬다, 2026-09-08).
case "$FILE" in
...
KIND_NOTES = {
    "member-ui": (
        "📱 멤버 화면을 변경했습니다. 이 서비스는 모바일에서 주로 쓰입니다.\n"
    ...
    "admin-ui": (
        ...
        "- 멤버 화면 톤을 가져오지 마세요. `.admin-surface` 의 중성 팔레트를 씁니다.\n"
    ...
    "user-guide": (
        ...
        "- `docs/guide/` 는 멤버와 주선자가 읽는 문서입니다. 화면 흐름·버튼·정책 숫자\n"
