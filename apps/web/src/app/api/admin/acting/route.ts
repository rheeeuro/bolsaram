 ...
 *
 * 경계는 두 곳에서 중복으로 본다.
 *   - 여기: `app_can_edit_profile` + 아직 연결되지 않은 프로필인지
 *   - RLS : `app_current_profile_id()` 가 요청마다 같은 조건을 다시 본다(0025)
 */
...
export const POST = route(async (request: Request) => {
  ...
  await withRls(rlsContextOf(viewer), async (sql) => {
    ...
    // 본인이 자기 계정으로 들어와 쓰고 있는 프로필은 대행하지 않는다.
    ...
    if (row.claimed) {
      throw new DomainError(
        "FORBIDDEN",
        "본인 계정이 연결된 프로필은 대신 볼 수 없습니다. 본인에게 직접 확인해 주세요.",
      );
    }
