...
export const messages = {
  ...
  /**
   * 다른 주선자 계정에 붙어 있던 텔레그램 계정을 옮겨왔을 때. 옮겨온 사실을 반드시
   * 알린다 — 이전 계정의 봇은 이 순간부터 멈추고, 그 계정에서 올리던 등록도 닫힌다.
   */
  linkedMoved: [
    "연결됐습니다.",
    "",
    "이 텔레그램 계정은 다른 주선자 계정에 연결돼 있었습니다.",
    "그 연결은 해제했고, 거기서 진행 중이던 등록은 취소했습니다.",
    "",
    "프로필 사진을 보내고, 이어서 프로필 글을 보내주세요.",
    "",
    "/status 진행 상황 · /new 새로 시작 · /cancel 취소",
  ...
  /**
   * 멤버가 낸 요청 (0026). 아직 상대에게 가지 않았다 — 주선자가 확인해야 움직인다.
  ...
  /**
   * 요청이 보류됐다 (0031). 보류한 사람이 그 멤버의 담당자가 아닐 수 있어서,
   * **멤버에게 말해줄 사람**이 알아야 한다. 그래서 다음 할 일을 문장으로 적는다.
   */
  intentDeclined: (
    kind: MatchIntentKind | null,
    fromCode: number | null,
    ...
    [
      ...
      pairLine(fromCode, toCode, "→"),
      "",
      "상대에게 전달되지 않았습니다. 멤버에게 알려주세요.",
    ].join("\n"),

  /**
   * 신청이 거절됐다 (0032). 거절당한 쪽에는 화면 알림이 뜨지 않으므로
   * **그 멤버의 주선자**가 알고 전해야 한다.
   */
  ...
  /**
   * 모임 채팅방에 새 글이 있다 (0040). **내용은 싣지 않는다** — 알림은 방을
   * 가리키기만 하고 대화는 볼사람에서 읽는다. 여러 줄이 와도 아직 보내지 않은
   * 알림이 있으면 하나로 접히므로 개수도 말하지 않는다.
   */
  groupMessage: (groupName: string | null) =>
    [
      `${roomName(groupName)} 채팅방에 새 글이 있습니다.`,
      "",
      "볼사람에서 확인해 주세요.",
    ].join("\n"),
